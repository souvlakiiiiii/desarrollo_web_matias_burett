from sqlalchemy import create_engine, Column, Integer, BigInteger, String, ForeignKey, Enum, DateTime, func, desc
...
class ContactarPor(Base):
    ...
    id=Column(Integer, nullable=False, primary_key=True, autoincrement=True)
    nombre=Column(Enum('whatsapp', 'telegram', 'X', 'instagram', 'tiktok', 'bluesky'), nullable=False)
...
def get_datos_grafico1():
    session=SessionLocal()
    semana_ant = datetime.now() - timedelta(days=7)
    cantidad = (session.query(func.date(AvisoAdopcion.fecha_ingreso).label('fecha'), func.count(AvisoAdopcion.id).label('cantidad'))
                .filter(AvisoAdopcion.fecha_ingreso >= semana_ant).group_by('fecha').order_by(desc('fecha')).all())
    datos = []
    for fecha_str, cantidad in cantidad:
        fecha_dt = datetime.strptime(str(fecha_str), '%Y-%m-%d')
        timestamp_ms = int(fecha_dt.timestamp() * 1000) 
        datos.append([timestamp_ms, cantidad])
    session.close()
    return datos

def get_datos_grafico2():
    session=SessionLocal()
    cperros = session.query(AvisoAdopcion).filter_by(tipo='perro').count()
    cgatos = session.query(AvisoAdopcion).filter_by(tipo='gato').count()
    datos = [
            {"name": "Perros", "y": cperros, "color": "#FF9900"},
            {"name": "Gatos", "y": cgatos, "color": "#00AADD"}
        ]
    session.close()
    return datos

def get_datos_grafico3():
    session=SessionLocal()
    conteos = (session.query(func.extract('month', AvisoAdopcion.fecha_ingreso).label('fecha'), AvisoAdopcion.tipo,
                             func.count(AvisoAdopcion.id).label('conteo'))
                             .group_by(AvisoAdopcion.tipo, func.extract('month', AvisoAdopcion.fecha_ingreso)).all())
    
    meses = {1: 'Enero', 2: 'Febrero', 3: 'Marzo', 4: 'Abril', 5: 'Mayo', 6: 'Junio',
                 7: 'Julio', 8: 'Agosto', 9: 'Septiembre', 10: 'Octubre', 11: 'Noviembre', 12: 'Diciembre'}
        
        
    datos_mensuales = {'perro': {m: 0 for m in range(1, 13)}, 'gato': {m: 0 for m in range(1, 13)}}
        
    for mes_num_float, tipo, cantidad in conteos:
        mes_num = int(mes_num_float)
        if tipo in datos_mensuales:
            datos_mensuales[tipo][mes_num] = cantidad
                
    categories = list(meses.values())
    dperros = [datos_mensuales['perro'][m] for m in range(1, 13)]
    dgatos = [datos_mensuales['gato'][m] for m in range(1, 13)]
        
    datos = {
        "categories": categories,
        "series": [
            {"name": "Perros", "data": dperros},
            {"name": "Gatos", "data": dgatos}
        ]
    }
    session.close()
    return datos
...
def create_comentario_get_id(nombre, texto, fecha, aviso_id):
    session = SessionLocal()
    new_comentario=Comentarios(nombre=nombre, texto=texto, fecha=fecha, aviso_id=aviso_id)
    session.add(new_comentario)
    session.commit()
    aviso_id=new_comentario.id
    session.close()
    return aviso_id

def get_all_comentarios_by_aviso_id(id):
    session = SessionLocal()
