...
def validar_comentario(nombre, comentario):
    if len(nombre) < 3 or len(nombre) > 80 or len(comentario) < 5:
        return False
    return True
...
def mascota(aviso_id):
    ...
    return render_template("mascota.html", aviso=aviso, fotos=fotos, contacto=contacto, comentarios=comentarios)
        
@app.route("/comentarios/<int:aviso_id>")
def comentarios(aviso_id):
    todos_comentarios=get_all_comentarios_by_aviso_id(aviso_id)
    data=([{"id": c.id, "nombre": c.nombre, "text": c.texto, "fecha": c.fecha, c.fecha.strftime("%Y-%m-%d %H:%M:%S") 
            if c.fecha else None: c.aviso_id} for c in todos_comentarios])
    return jsonify(data)

@app.route("/comentarios/agregar/<int:aviso_id>", methods=["POST"])
def agregar_comentarios(aviso_id):
...
@app.route("/datos-graficos", methods=['GET'])
def datos_graficos():
    sesion=SessionLocal()
    grafico1 = get_datos_grafico1()
    grafico2 = get_datos_grafico2()
    grafico3 = get_datos_grafico3()
    diccionario = {1: grafico1, 2: grafico2, 3: grafico3}
    respuesta = jsonify({"status": "ok", "data": diccionario})
